			...
			foreach( $versions['versions']['item'] as $item ) 
			{
				...
				if ( $item['_attr']['name'] == strtolower( trim( $theme['Name'] ) ) ) 
				{
					...
					if ( $theme['Version'] < $cur ) 
					{
						...
						$notification .= '<p class="update">' . sprintf( __( 'There is a new version of your theme available (' . '%s' . '). Please visit', 'sp' ), $item['value'] ) . ' <a href="' . esc_url( 'http://splashingpixels.com/member-dashboard/' ) . '" title="Splashing Pixels" target="_blank">Splashing Pixels</a> to download. If your theme is not listed, you probably purchased it elsewhere.  In that case, please download it from where you purchased it.<a href="' . esc_url( 'http://splashingpixels.com/wp-content/themes/sp/versions/' . strtolower( trim( $spthemeinfo['Name'] ) ) . '_changelog.txt' ) . '" title="Theme Changelog" target="_blank">(ChangeLog)</a>(' . __( 'To disable this message permanently', 'sp' ) . ' <a href="#" class="hide_msg" title="Hide Message">' . __( 'click here', 'sp' ) . '</a>)</p>';
					}
				}
